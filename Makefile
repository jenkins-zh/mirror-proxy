...
image: linux
	docker build -t jenkins-zh/mirror-proxy:dev-$(COMMIT) .

image-github: linux
	docker build -t docker.pkg.github.com/jenkins-zh/mirror-proxy/mirror-proxy:dev-$(COMMIT) .

push-image: image
	docker push jenkins-zh/mirror-proxy:dev-$(COMMIT)
...
front-image:
	docker build -t jenkins-zh/mirror-proxy-front:dev-$(COMMIT) front
...
front-image-push: front-image
	docker push jenkins-zh/mirror-proxy-front:dev-$(COMMIT)
